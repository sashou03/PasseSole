...
async def creer_partie(ctx):
    ...
    # The creator is also added to the players right away or not? The requirements say:
    # "Il enregistre le créateur de la partie comme 'hôte' (owner) de cette partie."
    # Let's force them to !join if they want to play, or auto-join them.
    # The requirement says: "!join: Le joueur est ajouté à la liste...".
    # We will auto-join the owner just in case, or maybe require !join. Let's let them explicitly !join.
    await ctx.send(f"Partie créée par {ctx.author.mention} ! Utilisez `!join` pour la rejoindre et `!start` pour la lancer.")

@bot.command()
async def join(ctx):
    if ctx.channel.id not in games:
        await ctx.send("Aucune partie n'est en préparation dans ce salon. Utilisez `!creer_partie` pour en créer une.")
        return
    ...
    await ctx.send(f"{ctx.author.mention} a rejoint la partie ! ({len(game.players)}/15 joueurs)")
...
async def quit(ctx):
    ...
    if ctx.author.id == game.owner_id:
        ...
        if ctx.author.id in game.players:
            ...
            # Cancel their mission without penalty if any
...
async def start(ctx):
    if ctx.channel.id not in games:
        await ctx.send("Aucune partie n'est en préparation dans ce salon. Utilisez `!creer_partie`.")
        return
    ...
    # Announce some missions randomly to show possibilities
...
async def lancer_mission(ctx):
    ...
    game = games[ctx.channel.id]
    if not game.started:
        await ctx.send("La partie n'a pas encore commencé. Attendez que le créateur fasse `!start`.")
        return
        
    if ctx.author.id not in game.players:
        await ctx.send("Vous ne participez pas à cette partie. Utilisez `!join` si ce n'est pas complet.")
        return
        
    if ctx.author.id in game.active_missions:
        await ctx.send("Vous avez déjà une mission en cours ! Vous devez la réussir (`!mission_reussie`) ou l'abandonner (`!abandon`).")
        return
    ...
    await ctx.send(f"🎯 **Mission attribuée à {ctx.author.mention} !**\n\n📜 {text}\n\n⏰ Tu as 5 minutes pour accomplir cette mission.")
    ...
    async def timer_task():
        ...
        if hasattr(game, 'active_missions') and ctx.author.id in game.active_missions:
            ...
            if m == mission_state:
                ...
                await ctx.send(f"⏳ **Temps écoulé pour {ctx.author.mention} !**\nLa mission n'a pas été validée à temps. Pénalité: -1 point.")
...
async def mission_reussie(ctx):
    ...
    vote_msg = await ctx.send(f"📢 **Mission de {ctx.author.mention} terminée.**\n\nRappel : *{mission_state.text}*\n\nVotez pour confirmer si la mission est réussie ! (30s)")
    ...
    if yes_votes > no_votes:
        update_score(ctx.author.id, 1)
        await ctx.send(f"🎉 **Mission validée pour {ctx.author.mention} !** (+1 point) ✅ ({yes_votes} oui contre {no_votes} non)")
    else:
        update_score(ctx.author.id, -1)
        await ctx.send(f"❌ **Mission refusée pour {ctx.author.mention}.** (-1 point) ❌ ({yes_votes} oui, {no_votes} non)")
...
async def score(ctx):
    ...
    for idx, (uid, sc) in enumerate(sorted_scores[:20], 1): # Top 20 max
